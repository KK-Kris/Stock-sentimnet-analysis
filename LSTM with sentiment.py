...
LSTM = Sequential()
LSTM.add(Dense(64, input_shape=(X_train.shape[1], X_train.shape[2])))
LSTM.add(Dropout(0.4))
LSTM.add(Dense(1, activation='relu'))

LSTM.compile(loss='mse', optimizer='adam')
train_predict = LSTM.fit(X_train, Y_train, epochs=50, batch_size=30, validation_data=(X_test, Y_test), shuffle=False, verbose=0)
...
Y_predict = LSTM.predict(X_test)[:,0]
...
# plt.ylabel('Close')
# plt.savefig("picture/LSTM有1.png", dpi=500, bbox_inches='tight')  #  解决图片不清晰，不完整的问题
...
plt.show()
df = []
df.append(mean_absolute_error(Sentiment_Test, Sentiment_Predict))
df.append(mean_squared_error(Sentiment_Test, Sentiment_Predict))
df.append(sqrt(mean_squared_error(Sentiment_Test, Sentiment_Predict)))
df.append(r2_score(Sentiment_Test, Sentiment_Predict))

# with open('picture/LSTM_YES_Result.csv', 'w') as f:
#     for word in df:
#         f.write("%s" % word)
#
